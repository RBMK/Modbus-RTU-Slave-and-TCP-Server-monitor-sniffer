Simple Modbus RTU slave and TCP server with selection of data types and other useful options not seen in other utilities.
It can act as gateway with multiple IDs, and serve various data types and monitor or sniff them.
Data can be saved to CSV and later reloaded.

This program comes with absolutely no warranty. 
It can be unpredictable because of coded with Gemini. 
Try at your own risk for educational purpose or tinkering.

If you find it useful, give me a beer, please. 
https://buymeacoffee.com/technokneznik

Heavily inspired by https://github.com/ClassicDIY/ModbusTool
Please send him a coffee and try his utility first, it is more user friendly.
